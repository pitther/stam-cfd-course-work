...
const useStemFluid = ({SIZE, visc, diff}) => {
    SOLID_OBJECTS = new Array((SIZE + 2) * (SIZE + 2)).fill(false);
    const FLUID = new Fluid(SIZE, visc, diff);

    const addSolidObject = (x, y) => {
        SOLID_OBJECTS[IX(SIZE, x, y)] = true;
    }

    const removeSolidObject = (x, y) => {
        SOLID_OBJECTS[IX(SIZE, y, x)] = false;
    }

    return {FLUID, addSolidObject, removeSolidObject, SOLID_OBJECTS}
}

export {useStemFluid, IX}
